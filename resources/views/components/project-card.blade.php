...
<div
    class="project-card group relative w-full h-80 rounded-2xl overflow-hidden cursor-pointer border border-white/10 hover:border-cyan-500/50 transition-all duration-500 shadow-lg hover:shadow-[0_0_30px_rgba(34,211,238,0.2)] bg-slate-900/50"
    onclick="toggleMobileCard(this, event)"
>
    
    <!-- Background Image or Gradient Placeholder -->
    @if($image)
        <img src="{{ $image }}" alt="{{ $title }}" class="absolute inset-0 w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 group-hover:rotate-1 opacity-70 group-hover:opacity-30 card-img">
    @else
        <div class="absolute inset-0 bg-linear-to-br from-slate-800 to-slate-900 transition-transform duration-700 group-hover:scale-110 card-bg">
            <!-- Placeholder pattern -->
    ...
    <!-- Overlay escura que surge no hover / active -->
    <div class="card-overlay absolute inset-0 bg-slate-950/20 group-hover:bg-slate-950/80 transition-all duration-500 backdrop-blur-[1px] group-hover:backdrop-blur-md"></div>

    <!-- Conteúdo Estático (Sempre Visível, some/sobe no hover / active) -->
    <div class="card-static absolute inset-0 p-6 flex flex-col justify-end transition-all duration-500 group-hover:-translate-y-4 opacity-100 group-hover:opacity-0">
    ...
    <!-- Conteúdo Revelado (Oculto por padrão, desliza no hover / active) -->
    <div class="card-reveal absolute inset-0 p-6 flex flex-col justify-center translate-y-8 group-hover:translate-y-0 transition-all duration-500 opacity-0 group-hover:opacity-100 z-20">
        ...
        <a href="{{ $link }}" target="_blank" onclick="event.stopPropagation()" class="inline-flex items-center gap-2 text-sm font-bold text-white hover:text-cyan-300 transition-colors w-max group/btn">
            Ver Projeto
    ...
    <div class="absolute top-0 left-0 w-6 h-6 border-t-2 border-l-2 border-cyan-500/0 group-hover:border-cyan-400/80 transition-all duration-500 m-4 rounded-tl pointer-events-none card-corner-tl"></div>
    <div class="absolute bottom-0 right-0 w-6 h-6 border-b-2 border-r-2 border-blue-500/0 group-hover:border-blue-400/80 transition-all duration-500 m-4 rounded-br pointer-events-none card-corner-br"></div>
</div>

@once
<style>
    /* Mobile: card-active simula o efeito do hover */
    @media (hover: none) {
...
<script>
    function toggleMobileCard(card, event) {
        // Só age em dispositivos sem hover real (touch)
    ...
    // Fecha o card ao clicar fora (mobile)
